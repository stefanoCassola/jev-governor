...
def main() -> None:
    ...
    ap.add_argument("--ack", action="store_true",
                    help="v4a (async only): count the settle window from the iteration at which the solver "
                         "applied the last change, and allow one change in flight until then (guard exempt)")
    ...
    if a.ack and not (a.async_ and ctl3 is not None):
        sys.exit("--ack needs --async and a v3/v4 controller")
    ...
    if a.async_:
        ...
        next_n = a.interval
        in_flight = None      # --ack: decision iteration of a change the solver has not applied yet
        while True:
            rc = proc.poll()
            log.poll()
            if rc is not None:
                break
            if a.ack and in_flight is not None:
                ack = next((k for k in log.async_applied if k >= in_flight), None)
                if ack is not None:
                    # The new factors act from iteration ack+1, as after a synchronous decision at ack.
                    ctl3.last_change = ack
                    next_n = max(next_n, ack + a.interval)
                    in_flight = None
                elif not (log.iters and ctl3.guard_would_fire(log.iters[-1])):
                    time.sleep(0.02)
                    continue
            if not log.iters or log.iters[-1].it < next_n or log.converged:
                if not (a.ack and in_flight is not None):
                    time.sleep(0.02)
                    continue
            n = log.iters[-1].it
            f_before = dict(f)
            decide_at(n, list(log.iters))
            if a.ack and (f["U"], f["p"]) != (f_before["U"], f_before["p"]):
                in_flight = n
            next_n = n + a.interval
    else:
        served = set()
    ...
    result = {
        ...
        "decisions": n_dec, "host": os.uname().nodename, "async": a.async_, "ack": a.ack,
