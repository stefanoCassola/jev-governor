...
class Controller3:
    ...
    def __init__(self, judge, prof: dict, b: dict, N: int, ceiling_ttl: int | None = None):
        """ceiling_ttl=None: permanent ceiling after a stall back-off (v3); an integer: the ceiling
        expires after that many iterations (v4), because the stability landscape of alpha_U need not
        be monotonic (a larger step can converge where a smaller one stalls)."""
        self.judge, self.prof, self.b, self.N = judge, prof, b, N
        self.ceiling_ttl = ceiling_ttl
        self.ceiling_set_at: int | None = None
        self.cooldown = 0
    ...
    def guard_would_fire(self, it) -> bool:
        """Same test as the hard guard in decide(), without side effects (used while a change is in flight)."""
        if any((not math.isfinite(v)) for v in it.res.values()):
            return True
        return bool(self.last_res) and any(k in self.last_res and self.last_res[k] and v > 10 * self.last_res[k]
                                           and v > 1e-3 for k, v in it.res.items())
    ...
    def decide(self, iters, n: int, f: dict, hist: list) -> tuple[dict, dict]:
        ...
        if self.ceiling is not None and self.ceiling_ttl is not None and n - self.ceiling_set_at >= self.ceiling_ttl:
            self.ceiling = None            # v4: let the controller probe larger steps again
        guard = None
        ...
        if move == "decrease_small" and probs.get("stuck_high", 0) > 0.6 and not probs["diverging"] > 0.6:
            self.ceiling = nu            # do not climb back into the stalling range (for ceiling_ttl its in v4)
            self.ceiling_set_at = n
