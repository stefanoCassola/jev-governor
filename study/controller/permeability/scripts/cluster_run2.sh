...
# SOLVER_BIN: solver executable; the patched build in solver/bin re-reads the permeability stop switch
# at runtime (the stock simpleFoamMod reads it once), which the Jev controller relies on.
SOLVER_BIN=${SOLVER_BIN:-simpleFoamMod}
...
cd "$RUN"
{ echo "host $(hostname) job ${SLURM_JOB_ID:-none}"; lscpu | grep 'Model name'; grep -E '^\s+(U|p)\s' system/fvSolution; echo "solver $SOLVER_BIN"; } > run_info
date +%s.%N > start_epoch
stdbuf -oL -eL apptainer exec --bind /scratch,/work "$SIF" openfoam2312 "$SOLVER_BIN" -case "$RUN" > log.simpleFoamMod 2>&1 &
SOLVER=$!
if [ $JEV = 1 ]; then
